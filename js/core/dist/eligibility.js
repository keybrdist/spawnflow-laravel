...
export function passes(condition, data) {
...
export function references(condition) {
    const refs = [];
...
function evaluate(node, data) {
    ...
    if (Array.isArray(node))
        return node.map((item) => evaluate(item, data));
...
function evaluateVar(args, data) {
    ...
    if (value === MISSING) {
        if (fallback === MISSING)
            throw new InvalidConditionError(`var references absent key: ${path}`);
        return fallback;
    }
    return value;
}
function evaluateMissing(args, data) {
    const names = Array.isArray(args) ? args : [args];
    const absent = [];
    for (const name of names) {
        if (typeof name !== 'string')
            throw new InvalidConditionError('missing expects string field names');
        if (extract(name, data) === MISSING)
            absent.push(name);
    }
    return absent;
}
function evaluateIn(args, data) {
    const [needle, haystack] = operands(args, data, 'in');
    if (Array.isArray(haystack))
        return haystack.some((candidate) => equals(needle, candidate));
    if (typeof haystack === 'string' && typeof needle === 'string') {
        return needle !== '' && haystack.includes(needle);
    }
    throw new InvalidConditionError('in expects an array haystack, or string needle and haystack');
}
function binary(args, data, compare) {
    const [a, b] = operands(args, data, 'comparison');
    return compare(a, b);
}
function numeric(args, data, compare) {
...
function logical(args, data, requireAll) {
...
function operands(args, data, operator) {
...
function collectReferences(node, refs) {
    ...
    for (const key of keys)
        collectReferences(node[key], refs);
}
// ---------------------------------------------------------------
 ...
 */
function envelopeOutcome(rule, data, axis) {
    const governsVisibility = rule.effect === 'show' || rule.effect === 'hide';
    if ((axis === 'visible') !== governsVisibility)
        return null;
...
export function resolveRules(rules, data) {
    let visible = true;
    let enabled = true;
 ...
 */
export function fieldVerdicts(form, values) {
    ...
    for (const group of form.groups) {
        if (group.serverResolved) {
            ...
            if (shipped)
                groupVerdict.set(group.name, shipped);
            continue;
        }
...
export function completeData(fields, values) {
    const data = {};
